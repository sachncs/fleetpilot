...
import { loadConfig, saveConfig, type FleetPilotConfig } from './config-store';
...
export async function runOnboarding(): Promise<FleetPilotConfig> {
  ...
  let config: FleetPilotConfig;

  if (!process.stdin.isTTY) {
    console.log('[FleetPilot] Non-interactive environment detected — using default settings.');
    config = { ...base, port: 3000, maxConcurrentSolves: 1, maxTimeMs: 30_000, maxGenerations: 500 };
  } else {
    console.log('');
